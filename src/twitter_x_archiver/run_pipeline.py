...
from .collector import config_load
...
def main():
    ...
    cfg = config_load(app_paths.ensure_config())
    data = app_paths.resolve_data_dir(cfg)
    ...
    try:
        if args.after_probe_pid:
            ...
            expected = (b'twitter_x_archiver.collector', b'collect_zevent2026.py')
            while True:
                try:
                    cmd = Path(f'/proc/{args.after_probe_pid}/cmdline').read_bytes()
                except FileNotFoundError:
                    break
                if not any(name in cmd for name in expected) or b'probe' not in cmd:
                    break
                time.sleep(5)
            ...
            for mode in ('smoke','probe'):
                child = subprocess.Popen([sys.executable,'-m','twitter_x_archiver.collector','--mode',mode],cwd=ROOT)
                save(phase=mode,child_pid=child.pid)
                code = child.wait()
                if code:
                    save(status='failed',exit_code=code)
                    return code
        child = subprocess.Popen([sys.executable,'-m','twitter_x_archiver.collector'],cwd=ROOT)
