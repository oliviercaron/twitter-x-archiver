#!/usr/bin/env python3
"""Twitter/X collection and metadata preservation. Never pass cookies as command-line arguments."""
import argparse
import asyncio
import copy
import dataclasses
...
def plan(config):
    ...
    if not path.exists():
        path = app_paths.assets()/'defaults'/config['streamers_file']
...
def main():
    ...
    try:
        ...
        root=(config['_root']/config['data_dir']).resolve() if args.config else app_paths.resolve_data_dir(config)
