"""Start, stop and inspect the local archive service across operating systems.

The HTTP health endpoint is authoritative: status files may survive a crash.
Source installations run the Python package; frozen releases restart their own
executable. Process records include the platform to distinguish Windows and WSL."""
import json
import os
import signal
import subprocess
...
def command():
    """Restart the current app, using the installed package in source mode."""
    if getattr(sys, 'frozen', False):return [sys.executable]
    python=VENV_PYTHONW if WINDOWS and VENV_PYTHONW.exists() else VENV_PYTHON if VENV_PYTHON.exists() else Path(sys.executable)
    return [str(python), '-m', 'twitter_x_archiver.server']
