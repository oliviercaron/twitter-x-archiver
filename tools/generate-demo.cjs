...
// node tools/generate-demo.cjs /path/to/output
...
(async()=>{
 ...
 await page.route('**/*',async route=>{
  ...
  return route.fulfill({contentType:filename.endsWith('.js')?'text/javascript':filename.endsWith('.css')?'text/css':'text/html',body:fs.readFileSync(path.join(root,'manual_ui',filename),'utf8')});
 });
 try{
