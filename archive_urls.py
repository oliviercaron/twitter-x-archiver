...
def main():
    ...
    parser.add_argument('--inclure-replies','--include-replies',dest='include_replies',action='store_true',help='Archiver aussi les réponses et leurs chaînes accessibles')
    ...
    if not urls:
        print('Aucune URL de post valide. Lignes invalides :',len(invalid));return 2
    jobs=Jobs(data)
    ...
    print(f"{len(ids)} URL uniques prises en compte ; {len(invalid)} lignes invalides.")
    print('État :', {state:sum(j['status']==state for j in summary['jobs']) for state in sorted({j['status'] for j in summary['jobs']})})
    print('Rapport :',path)
    if server:print('La file est traitée par le serveur local. Suivi : http://127.0.0.1:18765')
    return 1 if not server and any(j['status']!='done' for j in summary['jobs']) else 0


if __name__=='__main__':
    try:sys.exit(main())
    except KeyboardInterrupt:print('Interrompu ; la file est conservée.');sys.exit(130)
    except Exception as exc:print('Échec :',type(exc).__name__,'; aucun secret affiché.');sys.exit(1)
