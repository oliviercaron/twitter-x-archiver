...
from collect_zevent2026 import config_load,single_writer
        ...
        def get_content(self):
            ...
            if path=='/api/paths':
                ...
                return self.respond(200,{'project':display_path(ROOT),'data':display_path(data_dir),
                                         'extension':display_path(ROOT/'chrome-extension'),
                                         'posts':display_path(data_dir/'posts')})
            if path=='/':
                html=(ASSETS/'manual_ui/index.html').read_text(encoding='utf-8').replace('__BRIDGE_TOKEN__',token)
            ...
            if path in ('/ui.js','/ui.css','/strings.js'):
                return self.respond(200,(ASSETS/'manual_ui'/path[1:]).read_text(encoding='utf-8'),
...
def main():
    ...
    with ExitStack() as locks:
        ...
        status(data,'running')
        print('Archivage manuel prêt : http://127.0.0.1:18765',flush=True)
        class WorkerStop:
...
def dispatch(argv):
    ...
    if argv and argv[0] in ('--install','--uninstall'):
        import install_native_host, webbrowser, server_control
        code=install_native_host.main(['--uninstall'] if argv[0]=='--uninstall' else [])
        if argv[0]=='--install' and code==0:
            print()
            print("Une seule etape reste a faire a la main :")
            print("  1. ouvrez chrome://extensions et activez le Mode developpeur")
            print("  2. cliquez sur Charger l'extension non empaquetee")
            print(f"  3. choisissez le dossier {app_paths.home()/'chrome-extension'}")
            print()
            print("Le service demarre, la page va s'ouvrir.")
...
if __name__=='__main__':
    try:raise SystemExit(dispatch(sys.argv[1:]) or 0)
    except SystemExit:raise
    except app_paths.StoragePathError as exc:print(str(exc));raise SystemExit(1)
    except Exception as exc:print(f'Le service s’est arrêté (erreur : {type(exc).__name__}). Fermez toute autre fenêtre '
          f'd’archivage ouverte, puis relancez.');raise SystemExit(1)
